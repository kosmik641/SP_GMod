...
function UARTFrontViewClient:Initialize()
    self.ADC_Signals = {}
    self.Input_Signals = {}
    self.Output_Signals = {}
    self.Arrow_Signals = {}
    ...
    self.Number_Of_Controllers = 0
    self.Nmb_Of_ADC_Signals = 0
    self.Nmb_Of_Arrow_Signals = 0
    self.Nmb_Of_Indicator_Signals = 0
    self.Nmb_Of_SAUT_Signals = 0
    self.Nmb_Of_7SegDec_Signals = 0
    self.Nmb_Of_Text_Displays = 0
    
    self.Config_Arrow_Signals = {}
    self.Config_SAUT_Signals = {}
    self.Config_Text_Signals = {}
    
    self.Config_Indicator_Signals = {}
    self.Config_7SegDec_Signals = {}
    self.Config_Signals_Length = 0
    self.Config_Signals = {}
    
    self._7SegDec_Signals = {}
    self.TextDisplaysSignalsStruct = {}
    ...
    self.Nmb_Of_ADC_Signals_Per_Contr = {}
    self.Nmb_Of_Arrow_Bytes_Per_Contr = {}
    self.Nmb_Of_Indicator_Bytes_Per_Contr = {}
    self.Nmb_Of_SAUT_Bytes_Per_Contr = {}
    
    self.Nmb_Of_UART_Bytes_Per_Contr = {}
    self.Nmb_Of_Text_Bytes_Per_Controller = {}
    self.Nmb_Of_Input_Signals = 0
    self.Nmb_Of_Output_Signals = 0
    self.Nmb_Of_Config_Bytes = 0
    self.Nmb_Of_UART_Bytes = 0
...
function UARTFrontViewClient:update()
    if CurTime() - self.lastUpdate < FrameTime()*2.1 then return end
    self.lastUpdate = CurTime()
    
    local command = self.command
    UART.WriteByte(Read_Command,2)
    local Input_Bytes = UART.ReadByte(self.Nmb_Of_Input_Bytes+3) --51 с 14 МК, 39 с 10 МК
    
    if (Input_Bytes[0] == 0x00 and Input_Bytes[1] == 0x0D) then self.command = 2 end
    
    if command == 0 then
    -- Reading_Bytes
        if (Input_Bytes[0] == 0x0B and Input_Bytes[1] == 0x0B and Input_Bytes[2] == self.Nmb_Of_Input_Bytes) then -- Если эти байты удовлетворяют протоколу, работаем с ними
            self.lastGetTime = CurTime()
            ...
            for Cntr_Nmb = 0,self.Number_Of_Controllers-1 do
                for ADC_Nmb = 0,self.Nmb_Of_ADC_Signals_Per_Contr[Cntr_Nmb]-1 do
                    self.ADC_Signals[ADC_Signals_Index] = bit.lshift(Input_Bytes[Input_Bytes_Index],8); Input_Bytes_Index = Input_Bytes_Index + 1
                    self.ADC_Signals[ADC_Signals_Index] = bit.bor(self.ADC_Signals[ADC_Signals_Index],Input_Bytes[Input_Bytes_Index]); ADC_Signals_Index = ADC_Signals_Index + 1; Input_Bytes_Index = Input_Bytes_Index + 1
                end
                
                for Pin_Nmb = 0,23 do
                    self.Input_Signals[Input_Signals_Index] = bit.rshift(bit.band(bit.lshift(Input_Bytes[Input_Bytes_Index],(Pin_Nmb % 8)),0x80),7); Input_Signals_Index = Input_Signals_Index + 1
                    if (Pin_Nmb == 7 or Pin_Nmb == 15 or Pin_Nmb == 23) then Input_Bytes_Index = Input_Bytes_Index + 1 end
                end
            end
        end
        
        Metro81717Signals:update(self)
    -- Writing_Bytes
        local Output_Bytes = self.Output_Bytes
        Output_Bytes[0] = self.Nmb_Of_Output_Bytes
        Output_Bytes[1] = 0x83
        
        -- for Output_Signals_Index = 0,self.Nmb_Of_Output_Signals-1 do
        for Output_Signals_Index = 200,239 do --Оптимизация цикла
            local ind = math.floor(Output_Signals_Index / 8) + 2
            if (Output_Signals_Index % 8 == 0) then Output_Bytes[ind] = 0 end
            Output_Bytes[ind] = bit.bor(Output_Bytes[ind],bit.rshift(bit.lshift(self.Output_Signals[Output_Signals_Index],7),(Output_Signals_Index % 8)))
        end
        ...
        for Cntr_Nmb = 0,self.Number_Of_Controllers-1 do
            for Port_Nmb=0,2 do
                if (self.Config_7SegDec_Signals[Cntr_Nmb][Port_Nmb] == 1) then
                    Output_Bytes[Cntr_Nmb * 3 + Port_Nmb + 2] = self.Convert_Int_Number_To_8_Output_Signals(self._7SegDec_Signals[_7SegDec_Signals_Index])
                    _7SegDec_Signals_Index = _7SegDec_Signals_Index + 1
                end
            end
        end
        UART.WriteByte(Output_Bytes,self.Nmb_Of_Output_Bytes + 2)
        self.command = 1
    elseif command == 1 then
    -- Writing_UART_Bytes
        if (self.Nmb_Of_UART_Bytes > 0) then
            local UART_Bytes = {}
            UART_Bytes[0] = self.Nmb_Of_UART_Bytes
            UART_Bytes[1] = 0x87
            ...
            for Cntr_Nmb = 0,self.Number_Of_Controllers-1 do
                if (self.Nmb_Of_Arrow_Bytes_Per_Contr[Cntr_Nmb] > 0) then
                    UART_Bytes[UART_Bytes_Counter] = 0x41; --[["A" aka ARROWS]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                    for i=0,self.Config_Arrow_Signals[Cntr_Nmb]-1 do
                        UART_Bytes[UART_Bytes_Counter] = i+1; UART_Bytes_Counter = UART_Bytes_Counter + 1
                        UART_Bytes[UART_Bytes_Counter] = bit.rshift(bit.band(self.Arrow_Signals[Arrow_Signals_Index],0xFF00),8); UART_Bytes_Counter = UART_Bytes_Counter + 1
                        UART_Bytes[UART_Bytes_Counter] = bit.band(self.Arrow_Signals[Arrow_Signals_Index],0x00FF); UART_Bytes_Counter = UART_Bytes_Counter + 1; Arrow_Signals_Index = Arrow_Signals_Index + 1
                    end
                end
                ...
                -- if (self.Nmb_Of_Indicator_Bytes_Per_Contr[Cntr_Nmb] > 0) then
                    -- for Ind_Nmb=0,2 do
                        -- if (self.Config_Indicator_Signals[Cntr_Nmb].map[Ind_Nmb][0] > 0) then
                            -- UART_Bytes[UART_Bytes_Counter] = 0x49; --[["I" aka INDICATORS]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                            -- UART_Bytes[UART_Bytes_Counter] = tobyte(Ind_Nmb+1); UART_Bytes_Counter = UART_Bytes_Counter + 1
                            -- UART_Bytes[UART_Bytes_Counter] = tobyte(self.Config_Indicator_Signals[Cntr_Nmb].map[Ind_Nmb][0]); UART_Bytes_Counter = UART_Bytes_Counter + 1
                            ...
                            -- for Indicator_Bytes_Index = 0,self.Config_Indicator_Signals[Cntr_Nmb].map[Ind_Nmb][0]-1 do
                                -- UART_Bytes[UART_Bytes_Counter] = Digits[Indicator_Bytes_Index]; UART_Bytes_Counter = UART_Bytes_Counter + 1
                            -- end
                            
                            -- UART_Bytes[UART_Bytes_Counter] = 0xFF; UART_Bytes_Counter = UART_Bytes_Counter + 1
                        -- end
                    -- end
                -- end
                
                -- if (self.Nmb_Of_SAUT_Bytes_Per_Contr[Cntr_Nmb] > 0) then
                    -- UART_Bytes[UART_Bytes_Counter] = 0x4C; --[["L" aka LEDS]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                    -- UART_Bytes[UART_Bytes_Counter] = tobyte(bit.bor(bit.lshift(self.SAUT_Signals[SAUT_Signals_Counter],2),bit.lshift(self.SAUT_Signals[SAUT_Signals_Counter + 1],1),self.SAUT_Signals[SAUT_Signals_Counter + 2]))
                    -- SAUT_Signals_Counter = SAUT_Signals_Counter + 3
                -- end
                
                if (self.Nmb_Of_Text_Bytes_Per_Controller[Cntr_Nmb] > 0) then
                    UART_Bytes[UART_Bytes_Counter] = 0x54;--[[ "T" aka TEXT ]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                    UART_Bytes[UART_Bytes_Counter] = 0x42;--[[ "B" aka BEGIN ]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                    if (self.TextDisplaysSignalsStruct[Text_Display_Counter].DisplayOn == 1) then
                        if (self.TextDisplaysSignalsStruct[Text_Display_Counter].DisplayLedOn == 1) then
                            UART_Bytes[UART_Bytes_Counter] = 0x11;--[[ Включены экран и подсветка. ]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                        else
                            UART_Bytes[UART_Bytes_Counter] = 0x01;--[[ Включен только экран. ]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                        end
                    else
                        UART_Bytes[UART_Bytes_Counter] = 0x00;--[[ Выключен экран. ]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                    end
                    local temp = (self.Config_Text_Signals[Cntr_Nmb] * 2);
                    UART_Bytes[UART_Bytes_Counter] = temp; UART_Bytes_Counter = UART_Bytes_Counter + 1
                    for Text_Byte_Index = 0,(self.Config_Text_Signals[Cntr_Nmb] * 2) - 1 do
                        UART_Bytes[UART_Bytes_Counter] = self.TextDisplaysSignalsStruct[Text_Display_Counter].textBytes[Text_Byte_Index]; UART_Bytes_Counter = UART_Bytes_Counter + 1
                    end
                    UART_Bytes[UART_Bytes_Counter] = 0x54;--[[ "T" aka TEXT ]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                    UART_Bytes[UART_Bytes_Counter] = 0x45;--[[ "E" aka END ]] UART_Bytes_Counter = UART_Bytes_Counter + 1

                    Text_Display_Counter = Text_Display_Counter + 1
                end
            end
            UART.WriteByte(UART_Bytes,self.Nmb_Of_UART_Bytes + 2)
        end 
        self.command = 0
    elseif command == 2 then
        if not self.ACK_Bytes then
            self:ConfigSignals(false)
            self.command = 3
        end
    ...
    if CurTime() - self.lastGetTime > 1.5 then
        ULib.tsayError(nil, "UARTFrontViewClient.lua: Контроллер погиб, необходим перезапуск")
        self:COM_Close()
...
function UARTFrontViewClient:ConfigSignals(new_config) --Config_Signals
    if new_config then
        self.Nmb_Of_Input_Bytes = 0
        self.Nmb_Of_Output_Bytes = 0
        ...
        repeat
            if ((i + 1) % 25 ~= 0) then -- 24 бита отвечают за конфигурирование пинов контроллера, на шаге, кратном 25, записываем байт-показатель количества байт, передаваемых данным контроллером по UART
                ...
                for Ind_Nmb=0,2 do
                    self.Nmb_Of_Indicator_Bytes_Per_Contr[Cntr_Counter] = self.Nmb_Of_Indicator_Bytes_Per_Contr[Cntr_Counter] + ((self.Config_Indicator_Signals[Cntr_Counter][Ind_Nmb][0] == 0) and 0 or (Config_Indicator_Signals[Cntr_Counter][Ind_Nmb][0] + 4))
                end
...
-- Writing_UART_Bytes
    if (self.Nmb_Of_UART_Bytes > 0) then
        ...
        local UART_Bytes = {}
        UART_Bytes[0] = tobyte(self.Nmb_Of_UART_Bytes)
        UART_Bytes[1] = 0x87
        ...
        for Cntr_Nmb = 0,self.Number_Of_Controllers-1 do
            if (self.Nmb_Of_Arrow_Bytes_Per_Contr[Cntr_Nmb] > 0) then
                UART_Bytes[UART_Bytes_Counter] = 0x41; --[["A" aka ARROWS]] UART_Bytes_Counter = UART_Bytes_Counter + 1
                for i = 0,self.Config_Arrow_Signals[Cntr_Nmb]-1 do
                    UART_Bytes[UART_Bytes_Counter] = tobyte(i + 1); UART_Bytes_Counter = UART_Bytes_Counter + 1
					UART_Bytes[UART_Bytes_Counter] = 0x00; UART_Bytes_Counter = UART_Bytes_Counter + 1
					UART_Bytes[UART_Bytes_Counter] = 0x00; UART_Bytes_Counter = UART_Bytes_Counter + 1
                end
            end
            ...
            if (self.Nmb_Of_Indicator_Bytes_Per_Contr[Cntr_Nmb] > 0) then
                for Ind_Nmb = 0,2 do
                    if (self.Config_Indicator_Signals[Cntr_Nmb].map[Ind_Nmb][0] > 0) then
                        UART_Bytes[UART_Bytes_Counter] = 0x49; --[["I" aka INDICATORS]] UART_Bytes_Counter = UART_Bytes_Counter + 1
						UART_Bytes[UART_Bytes_Counter] = tobyte(Ind_Nmb+1); UART_Bytes_Counter = UART_Bytes_Counter + 1
						UART_Bytes[UART_Bytes_Counter] = tobyte(Config_Indicator_Signals[Cntr_Nmb].map[Ind_Nmb][0]); UART_Bytes_Counter = UART_Bytes_Counter + 1
                        
                        for Indicator_Bytes_Index = 0,self.Config_Indicator_Signals[Cntr_Nmb].map[Ind_Nmb][0]-1 do
                            UART_Bytes[UART_Bytes_Counter] = 0x0A; UART_Bytes_Counter = UART_Bytes_Counter + 1
                        end
